...
private:
   ...
   *
   * Use classic GDSF scaling (freq/size) to avoid biasing too much toward tiny objects,
   * but boost frequency increment on hit for smaller objects to help recency.
   * Scaling factor 1.0e6 keeps differences significant for frequency.
   */
...
public:
   ...
   *
   * If the object is found, its frequency is incremented (more for smaller objects),
   * its priority is recalculated, and its position in the priority queue is updated.
   *
  ...
  bool find(obj_id_t obj_id) {
    ...
    // Frequency increment: more for small objects (to encourage recency for hot small objects)
    if (obj.size <= 4096)
      obj.frequency += 2;
    else
      obj.frequency += 1;
  ...
  void insert(obj_id_t obj_id, uint64_t obj_size) {
    ...
    // Boost initial frequency for very small objects so they get more chances, and for very large objects so they're not evicted instantly
    if (obj_size <= 1024)
      new_obj.frequency = 2;
